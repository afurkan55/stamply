...
export default function OwnerPanel() {
  ...
  // ====================================================
  // REAL-TIME: subscribe to stamp_requests on mount
  // ====================================================
  useEffect(() => {
    ...
    channelRef.current = supabase
      ...
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'stamp_requests' },
        (payload) => {
      ...
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'stamp_requests' },
        (payload) => {
          if (payload.new.status !== 'pending') {
      ...
      .on(
        'postgres_changes',
        { event: 'DELETE', schema: 'public', table: 'stamp_requests' },
        (payload) => {
  ...
  async function approveRequest(req) {
    ...
    await supabase.from('stamps').insert({
      ...
      note: 'QR stamp request',
    })

    await supabase
      .from('customers')
      .update({ stamp_count: cust.stamp_count + 1 })
      .eq('id', req.customer_id)
  ...
  const tabs = [
    { id: 'requests', icon: '📋', label: 'Requests', badge: pendingCount },
      ...
      <div className="flex bg-white border-b border-amber-100 sticky top-16 z-10 shadow-sm">
        {tabs.map(tab => (
          ...
          >
            ...
            {tab.badge > 0 && (
              <span className="absolute top-1.5 right-1/4 bg-red-500 text-white text-xs font-bold w-4 h-4 rounded-full flex items-center justify-center leading-none">
          ...
          <div className="mt-2 space-y-3">
            ...
            {requestsLoading ? (
              ...
              <div className="text-center py-14 text-gray-400">
                ...
                <p className="text-sm mt-1">Customers will appear here when they scan their QR code</p>
              </div>
            ) : (
              pendingRequests.map(req => {
                ...
                return (
                  <div
                    key={req.id}
                    className="bg-white rounded-2xl shadow-md p-4 border-l-4 border-amber-400"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <p className="font-bold text-gray-900 text-lg">{req.customer_name}</p>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 bg-green-600 text-white py-2.5 rounded-xl font-semibold hover:bg-green-700 active:scale-95 transition-all disabled:opacity-50 text-sm"
                      >
                      ...
                      <button
                        ...
                        disabled={isProcessing}
                        className="flex-1 bg-red-100 text-red-600 py-2.5 rounded-xl font-semibold hover:bg-red-200 active:scale-95 transition-all disabled:opacity-50 text-sm"
                      >
                        {isProcessing ? '...' : '❌ Reject'}
                      </button>
                    </div>
